...
import { Table, TableHeader, TableCell, TableBody, TableRow, TableFooter, TableContainer, Badge, Button, Pagination }
from '@windmill/react-ui'
import { Exclamation, EditIcon, SearchIcon, Arrow, AdvertenciaPedidoDevuelto, Inactivar, PlusCircle, } from
'../../icons';
...
import { alertEscribirMotivoInactivacion, showAlertCorrect, alertInactivar, showMotivoInactivacion } from
'../../helpers/Alertas'
...
function Pedidos() {
  ...
  const ENTREGADO = 3
  const DEVUELTO = 4
  const { pedidos, getPedidos, pedidosEmpleado, idUsuario, updateFase, disablePedido , getPedidosEmpleado} = usePedidos();
  ...
  const ES_ADMINISTRADOR = empleadoLogged != null ?
    empleadoLogged.idUsuarioNavigation.idRolNavigation.nombre.toLowerCase() == 'administrador' : null
  ...
  const pedidosAmostrar = ES_ADMINISTRADOR ? pedidos : pedidosEmpleado
  ...
  useEffect(() => {
    ...
    filteredData = filteredData.filter(pedido => pedido.isActivo === !inactivar &&
      pedido.idEstadoNavigation.nombre.toLowerCase().includes(filtrar.toLowerCase()) && pedido.idEstado !== ENTREGADO)
    setTotalResults(filteredData.length)

    setDataTable2(filteredData.slice((pageTable2 - 1) * resultsPerPage, pageTable2 * resultsPerPage)
    );

  }, [pedidosAmostrar, pageTable2, search, filtrar, inactivar])
      ...
      if (respuesta.isConfirmed) {
        ...
        if (responseMotivo.isConfirmed) {
          ...
          if (ES_ADMINISTRADOR) {
            ...
            await getPedidosEmpleado(idUsuario)
          }
...
<>
  ...
  <div className="flex ml-auto mb-6 w-full">
    ...
    <div>
      <div className=" relative w-full max-w-xl mr-6 focus-within:text-purple-500">
        ...
        <Input className={STYLE_INPUT.replace('pl-4', 'pl-8' )} placeholder="Buscar pedido" value={search}
          onChange={searcher} />
      </div>
    </div>
  </div>
  <TableContainer className="mb-8">
    <Table>
      ...
      <TableBody>
        ...
        <TableRow key={pedido.idPedido}>
          ...
          <TableCell>
            <Badge className="text-xs text-gray-600 dark:text-gray-400" type={pedido.isActivo ? "success" : "danger" }>
              {pedido.isActivo ? 'Activo' : 'Inactivo'}</Badge>
          </TableCell>
          <TableCell title={!ES_ACTIVO ? "No puedes cambiar la fase de un pedido que esta inactivo" : ES_RECIBIDO
            ? "Pasar el pedido a producción" : ES_EN_PRODUCCION ? "Pasar el pedido a entregado" : ES_ENTREGADO
            ? "El pedido esta entregado" : ES_DEVUELTO ? "No puedes cambiar la fase de un pedido que esta devuelto" :
            null}>
            <Button disabled={ES_ENTREGADO || ES_DEVUELTO || !ES_ACTIVO ? true : false} layout="link"
              className='ml-6 mr-6 pr-5' size="icon" aria-label="Edit" onClick={()=> cambiarFase(pedido,
              detallesAEditar, FASE)}>
          ...
          <TableCell>
            <div className="flex items-center space-x-2">
              ...
              {ES_RECIBIDO ? (
              <Button layout="link" size="icon" aria-label="Edit" onClick={()=> history.push('/app/editarPedido', {
                idPedido: pedido.idPedido, pedido: pedido, clientePedido: clientePedido
                })} >
              ...
              <Button layout="link" size="icon" aria-label="Edit" onClick={()=> inactivarPedido(pedido, pedido.isActivo
                ? false : true)} >
              ...
              <Button layout="link" size="icon" aria-label="Delete" onClick={()=>
                openModalDetallePedidoDevuelto(pedido)} >
                <AdvertenciaPedidoDevuelto className='text-yellow-500 w-5 h-5' aria-hidden="true" />
              </Button>
              ) : null}

              {!ES_ACTIVO ? (
              <Button title='Ver motivo de inactivación' onClick={()=>
                showMotivoInactivacion(pedido.motivoInactivacion)} layout="link" size="icon" aria-label="Delete" >
    ...
    <TableFooter>
      {totalResults > 0 && (
      <Pagination totalResults={totalResults} resultsPerPage={resultsPerPage} onChange={onPageChangeTable2}
        label="Table navigation" key={totalResults} />
      )}
    </TableFooter>
  </TableContainer>
  ...
  <ModalDetallePedido isOpen={modalIsOpenDetallePedido} isClose={closeModalDetallePedido} pedido={idPedido}
    isActivo={isPedidoActivo} />
  )}

  {modalIsOpenDetallePedidoDevuelto && (
  <ModalDetallePedidoDevuelto isOpen={modalIsOpenDetallePedidoDevuelto} isClose={closeModalDetallePedidoDevuelto}
    pedido={idPedido} />
  )}
